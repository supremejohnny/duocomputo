<template>
  <div class="category-task">
    <div class="top-bar">
      <router-link to="/category" class="back-btn">← 返回分类</router-link>
      <div class="progress-bar" v-if="!sessionDone">
        <div class="progress-fill" :style="{ width: progressPercent + '%' }"></div>
      </div>
      <span class="progress-text" v-if="!sessionDone">{{ progress.current }}/{{ progress.total }}</span>
    </div>

    <div v-if="loading" class="loading">
      <div class="loading-icon">📚</div>
      <p>正在准备「{{ categoryName }}」题目...</p>
    </div>

    <div v-else-if="noQuestions" class="empty">
      <p>该分类暂无题目</p>
      <router-link to="/category" class="btn btn-primary btn-block">返回分类</router-link>
    </div>

    <!-- Session complete (inline) -->
    <div v-else-if="sessionDone" class="done-section">
      <div class="done-icon">🎉</div>
      <h2 class="done-title">本轮完成！</h2>
      <p class="done-desc">「{{ categoryName }}」练习结束</p>
      <div class="done-buttons">
        <button class="btn btn-primary btn-block" @click="restartRound">再来一轮</button>
        <router-link to="/category" class="btn btn-outline btn-block">换个分类</router-link>
        <router-link to="/" class="btn btn-outline btn-block">返回首页</router-link>
      </div>
    </div>
...
const currentQuestion = computed(() => sessionDone.value ? null : getCurrentQuestion())
...
async function loadQuestions() {
  ...
  try {
    const res = await api.getCategoryDailyTask(categoryName.value)
    const mc = res.data.mcQuestions || []
    const fc = res.data.flashcardQuestions || []
    if (mc.length === 0 && fc.length === 0) {
...
.option-btn.selected { border-color: #1cb0f6; background: #e8f4fd; }
.option-btn.correct { border-color: #58cc02; background: #e6f9e6; }
.option-btn.wrong { border-color: #ff4b4b; background: #ffe6e6; }

.option-letter {
  width: 32px; height: 32px; border-radius: 8px;
  background: #f0f0f0; display: flex; align-items: center;
  justify-content: center; font-weight: 700; color: #777; flex-shrink: 0;
}
.option-btn.correct .option-letter { background: #58cc02; color: white; }
.option-btn.wrong .option-letter { background: #ff4b4b; color: white; }
.option-text { flex: 1; color: #333; }

.result-card { border-radius: 16px; padding: 20px; margin-bottom: 20px; }
.result-card.correct { background: #e6f9e6; border: 2px solid #58cc02; }
.result-card.wrong { background: #ffe6e6; border: 2px solid #ff4b4b; }
.result-header { font-size: 18px; font-weight: 700; margin-bottom: 8px; }
.result-explanation { font-size: 14px; color: #555; line-height: 1.6; }

/* --- Flashcard styles --- */
.card {
  background: white; border-radius: 20px; padding: 28px 24px;
  box-shadow: 0 4px 16px rgba(0,0,0,0.08); margin-bottom: 24px; min-height: 200px;
}
.card-front { text-align: center; }
.hint-text { color: #bbb; font-size: 14px; margin-bottom: 24px; }
.answer-label { font-size: 16px; color: #58cc02; margin: 0 0 16px; }
.answer-text { font-size: 14px; color: #444; line-height: 1.8; margin-bottom: 16px; }
...
.difficulty-label { font-size: 14px; color: #999; margin-bottom: 12px; }
.difficulty-buttons { display: flex; gap: 12px; }

.diff-btn {
  flex: 1; padding: 12px 8px; border-radius: 16px;
  border: 2px solid; font-size: 14px; font-weight: 600;
  cursor: pointer; transition: all 0.2s;
}
.diff-btn.easy { border-color: #58cc02; background: #e6f9e6; color: #58cc02; }
.diff-btn.easy:hover { background: #58cc02; color: white; }
.diff-btn.medium { border-color: #ff9600; background: #fff3e0; color: #ff9600; }
.diff-btn.medium:hover { background: #ff9600; color: white; }
.diff-btn.hard { border-color: #ff4b4b; background: #ffe6e6; color: #ff4b4b; }
.diff-btn.hard:hover { background: #ff4b4b; color: white; }

/* --- Shared --- */
.btn-block {
  display: block; width: 100%; padding: 14px; font-size: 16px;
  text-align: center; margin-top: 0; text-decoration: none;
}

.btn-outline {
  background: white; color: #58cc02; border: 2px solid #58cc02;
  border-radius: 16px; font-weight: 700;
}
.btn-outline:hover { background: #f0fff0; }
</style>
